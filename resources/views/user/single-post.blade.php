        ...
        <div class="card p-4" style="margin: 6rem 0;">
            ...
            @guest
                <div class="alert alert-secondary" style role="alert">
                    Yorum ekleyebilmek için giriş yapmanız gerekiyor! <a style="cursor:pointer;" href="{{route('login.show')}}"
                ...
                @foreach($comments as $comment)
                    <li class="media mb-5">
                        ...
                        <div class="media-body">
                            ...
                            @auth
                            <form class="mt-3" action="{{route('comment.answer.add')}}" method="post" autocomplete="off">
                                @csrf
                                <input type="text" name="commentContent" class="form-control" placeholder="Yanıtla..."/>
                                <input type="hidden" name="which_page" readonly value="{{$post->link}}">
                                <input type="hidden" name="commentId" readonly value="{{$comment->id}}">
                            </form>
                            @endauth
                            @guest
                                <a href="{{route('login.show')}}" class="mt-3 btn btn-primary w-100" style="cursor:text;background: initial;border: 1px solid #ced4da;border-radius: 0.25rem;text-align: left;font-size: 1rem;color: #6c757d;">Yanıtla...</a>
                            @endguest
                        </div>
                    </li>
                            ...
                            <li class="media mb-5" style="margin-left: 5rem;">
                                ...
                                <div class="media-body">
                                    <a href="/author/{{$commentAnswer->who_shared}}"
                                       class="mt-0 mb-1"><strong><span>@</span>{{$commentAnswer->who_shared}}</strong></a>
                                    ...
                                    <form class="mt-3" action="{{route('comment.answer.add')}}" method="post" autocomplete="off">
                                        ...
                                        <input type="text" name="commentContent" class="form-control" placeholder="Yanıtla..."/>
                                        <input type="hidden" name="which_page" readonly value="{{$post->link}}">
                                        <input type="hidden" name="commentId" readonly value="{{$commentAnswer->commentId}}">
                                    </form>
                                    @endauth
                                    @guest
                                        <a href="{{route('login.show')}}" class="mt-3 btn btn-primary w-100" style="cursor:text;background: initial;border: 1px solid #ced4da;border-radius: 0.25rem;text-align: left;font-size: 1rem;color: #6c757d;">Yanıtla...</a>
